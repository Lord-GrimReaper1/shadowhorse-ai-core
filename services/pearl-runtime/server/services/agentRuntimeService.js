...
function buildRuntimeInstruction(job) {
  return [
    ...
    'Work like a smart check loop, not a fixed checklist: after each tool result, decide whether the objective is answered, whether another required task was uncovered, or whether human approval/input is needed.',
    'If another required task is clearly necessary to complete the main objective and it is within the available tools and safety rules, continue to that task without asking the human to restate it.',
    'If the human corrects, narrows, cancels, or redirects the work, treat the newest human instruction as steering for the current run and adjust the answer or next step accordingly.',
    'Do not stop at a plan when the user asked you to execute and the next step is safe, discoverable, and available through tools. Do stop before writes, commits, installs, destructive actions, or actions requiring explicit approval.',
    'When useful, track discovered subtasks internally with pearl_create_task and pearl_complete_task, but keep task mechanics out of the final answer unless the human asks for progress details.',
