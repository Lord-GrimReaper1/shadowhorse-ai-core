...
test('simple conversational objectives stay in chat-only runtime', () => {
  ...
  assert.equal(runtime.isLikelyChatOnlyObjective('This is the answer you should have given me, Pearl: Prime Directives cannot be changed casually or at runtime.'), true);
  assert.equal(runtime.isLikelyChatOnlyObjective('The correct answer is that policy changes need dual-human approval.'), true);
...
test('implementation and project status objectives still use durable agent work', () => {
  ...
  assert.equal(runtime.isLikelyChatOnlyObjective('Implement this in code so Pearl stores the corrected answer.'), false);
